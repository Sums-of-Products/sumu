...
cdef extern from "gadget.h":
    ...
    uint64_t weight_sum_64(double w, uint64_t * psets, int m, double * weights, int j, int n, uint64_t U, uint64_t T, int t_ub)
    ...
    uint64_t weight_sum_128(double w, uint64_t * psets, int m, double * weights, int j, int n, vector[uint64_t] U, vector[uint64_t] T, int t_ub)
